...
- sqlite3 reports errors on stderr, so every `Process` in `data/Db.qml` collects stderr, and `errorText` in `data/Db.js` turns it into the message the toast and the journal show.
- A write that changes no row still exits 0. The writes that target one item print `SELECT changes()`, and `data/Db.qml` reports "item not found" when it reads 0.
