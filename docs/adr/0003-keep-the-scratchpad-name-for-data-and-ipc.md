...
Omanotes started as a copy of omatodolist, which stored everything in `scratchpad.db` and registered the IPC target `scratchpad`. Omanotes keeps both (`$XDG_DATA_HOME/omarchy/scratchpad.db` in `data/DbCore.qml`, `IpcHandler { target: "scratchpad" }` in `BarWidget.qml` and `ipcTarget: "scratchpad"` in `Panel.qml`) so notes already written and scripts already calling the target keep working after the switch.
